...
class Lock {
  ...
  private:
    ...
    int value;       // lock value, 1 or 0 (FREE or BUSY)
    Thread *owner;   // a pointer to the lock's owner
    List *queue;     // threads waiting in Acquire() for the value to be 1
};
...
enum RWlockStatus {NOP, WRITING, READING};
